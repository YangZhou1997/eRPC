...
#else
  if (kIsAzure) {
    // This routine gets the IPv4 address of the interface called ens2f0np0
    int fd = socket(AF_INET, SOCK_DGRAM, 0);
    struct ifreq ifr;
    ifr.ifr_addr.sa_family = AF_INET;
    strncpy(ifr.ifr_name, "ens2f0np0", IFNAMSIZ - 1);
    int ret = ioctl(fd, SIOCGIFADDR, &ifr);
    rt_assert(ret == 0, "DPDK: Failed to get IPv4 address of ens2f0np0");
    close(fd);
    return ntohl(
