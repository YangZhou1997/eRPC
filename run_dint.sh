...
def run_erpc_expr(app, num_of_uthreads):
    ...
    e = subprocess.Popen(['ssh', '-o', 'StrictHostKeyChecking=no', '-i', '~/.ssh/id_rsa', servers[0],
                          f"cd {erpc_dir_path} && autorun_app=small_rpc_tput sudo ./build/small_rpc_tput --test_ms 20000 --sm_verbose 0 --batch_size 1 --concurrency 256 --msg_size 0 --num_processes {num_processes} --numa_0_ports 2 --numa_1_ports 2 --process_id=0 --numa_node=1 --num_threads {num_cores} --is_client 0"],
                         shell=False,
    ...
    for i, client in enumerate(clients):
        ...
        e = subprocess.Popen(['ssh', '-o', 'StrictHostKeyChecking=no', '-i', '~/.ssh/id_rsa', client,
                              f"cd {erpc_dir_path} && autorun_app=small_rpc_tput sudo stdbuf -o0 ./build/small_rpc_tput --test_ms 20000 --sm_verbose 0 --batch_size 1 --concurrency 128 --msg_size 0 --num_processes {num_processes} --numa_0_ports 2 --numa_1_ports 2 --process_id={i + 1} --numa_node=1 --num_threads 64 --num_dst_threads {num_cores} --is_client 1"],
                             shell=False,
                             stdout=subprocess.PIPE,
                             stderr=subprocess.PIPE)
        executors.append(e)
    ...
    for e in executors:
        out, err = e.communicate()
        out = out.decode('utf-8').splitlines()[-50:]
        outputs.append(out)
